...
namespace cppcoro
{
	...
	public:
		...
		[[nodiscard]] auto join() noexcept
		{
			...
			public:
				...
				bool await_suspend(cppcoro::stdcoro::coroutine_handle<> continuation) noexcept
				{
	...
	private:
		...
		struct oneway_task
		{
			struct promise_type
			{
				cppcoro::stdcoro::suspend_never initial_suspend() { return {}; }
				cppcoro::stdcoro::suspend_never final_suspend() { return {}; }
