...
namespace cppcoro
{
	...
	public:
		round_robin_scheduler() noexcept
			: m_index(0)
			, m_noop(cppcoro::stdcoro::noop_coroutine())
		{
	...
	private:
		...
		cppcoro::stdcoro::coroutine_handle exchange_next(
			cppcoro::stdcoro::coroutine_handle<> coroutine) noexcept
		{
		...
		size_t m_index;
		const cppcoro::stdcoro::coroutine_handle<> m_noop;
		std::array<cppcoro::stdcoro::coroutine_handle<>, N - 1> m_coroutines;
	};
#endif
}
