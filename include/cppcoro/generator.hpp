...
namespace cppcoro
{
	...
	namespace detail
	{
		...
		public:
			...
			constexpr cppcoro::stdcoro::suspend_always initial_suspend() const { return {}; }
			constexpr cppcoro::stdcoro::suspend_always final_suspend() const { return {}; }

			template<
				typename U = T,
				std::enable_if_t<!std::is_rvalue_reference<U>::value, int> = 0>
			cppcoro::stdcoro::suspend_always yield_value(std::remove_reference_t<T>& value) noexcept
			{
		...
		{
			using coroutine_handle = cppcoro::stdcoro::coroutine_handle<generator_promise<T>>;
	...
	private:
		...
		explicit generator(cppcoro::stdcoro::coroutine_handle<promise_type> coroutine) noexcept
			: m_coroutine(coroutine)
		{}
