...
struct fork_join_operation
{
	...
	cppcoro::stdcoro::coroutine_handle<> m_coro;
	...
	bool await_suspend(cppcoro::stdcoro::coroutine_handle<> coro) noexcept
	{
		m_coro = coro;
...
cppcoro::task<void> for_each_async(SCHEDULER& scheduler, RANGE& range, FUNC func)
{
	...
	struct work_operation
	{
		...
		CPPCORO_NOINLINE
		void await_suspend(cppcoro::stdcoro::coroutine_handle<> coro) noexcept
		{
